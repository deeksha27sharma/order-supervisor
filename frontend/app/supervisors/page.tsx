    ...
    <div>
      ...
      {supervisors.length === 0 ? (
        <div style={{ border: "1px dashed var(--border-strong)", borderRadius: "var(--radius-lg)", padding: "48px 32px", textAlign: "center" }}>
          <p style={{ fontSize: "0.9rem", color: "var(--text-secondary)", marginBottom: 8 }}>No supervisor templates yet</p>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(380px, 1fr))", gap: 12 }}>
          {supervisors.map((s) => (
            <SupervisorCard key={s.id} supervisor={s} onDelete={handleDelete} />
          ))}
        </div>
      )}
    </div>
